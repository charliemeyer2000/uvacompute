...
│                                  ▼                                              │
│   ┌─────────────────────────────────────────────────────────────────────────┐  │
│   │              DigitalOcean VPS (Hub) - REDACTED-IP                       │  │
│   │                                                                         │  │
│   │   ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────────────┐│  │
...
The DO VPS (REDACTED-IP) currently runs:
...
- SSH access to DO VPS: `ssh root@REDACTED-IP`
...
- [x] **Install k3s server** on DO VPS with external access enabled
  ...
  curl -sfL https://get.k3s.io | sh -s - server \
    --disable=traefik \
    --tls-san=REDACTED-IP \
    --advertise-address=REDACTED-IP \
    --node-external-ip=REDACTED-IP
  ```
...
- [ ] **Update site environment variables** (USER ACTION REQUIRED)
  - `VM_ORCHESTRATION_SERVICE_URL=http://REDACTED-IP:8080`
...
{
  "success": true,
  "tunnelHost": "REDACTED-IP",
  "tunnelPort": 2223,
  "k3sUrl": "https://REDACTED-IP:6443",
