...
│                                  ▼                                              │
│   ┌─────────────────────────────────────────────────────────────────────────┐  │
│   │              DigitalOcean VPS (Hub) - ***REDACTED_IP***                       │  │
│   │                                                                         │  │
│   │   ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────────────┐│  │
...
The DO VPS (***REDACTED_IP***) currently runs:
...
- SSH access to DO VPS: `ssh root@***REDACTED_IP***`
...
- [x] **Install k3s server** on DO VPS with external access enabled
  ...
  curl -sfL https://get.k3s.io | sh -s - server \
    --disable=traefik \
    --tls-san=***REDACTED_IP*** \
    --advertise-address=***REDACTED_IP*** \
    --node-external-ip=***REDACTED_IP***
  ```
...
- [ ] **Update site environment variables** (USER ACTION REQUIRED)
  - `VM_ORCHESTRATION_SERVICE_URL=http://***REDACTED_IP***:8080`
...
{
  "success": true,
  "tunnelHost": "***REDACTED_IP***",
  "tunnelPort": 2223,
  "k3sUrl": "https://***REDACTED_IP***:6443",
...
| File                                          | Action | Description                   |
| --------------------------------------------- | ------ | ----------------------------- |
| `apps/vm-orchestration-service/lib/health.go` | Create | Node health monitoring        |
| `apps/site/convex/nodes.ts`                   | Modify | Add node health sync mutation |
| `apps/site/src/app/api/admin/health/route.ts` | Create | Health check endpoint         |

### Completion Criteria

- [ ] Node status synced from k8s to Convex
- [ ] Users notified when their workloads are affected
- [ ] VMs on dead nodes marked appropriately
- [ ] Jobs rescheduled when possible
- [ ] Admin can see node health in dashboard
