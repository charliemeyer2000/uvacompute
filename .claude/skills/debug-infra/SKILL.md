...
| ---------------- | -------------------------------------------------------- | ---------------------------------------------------- |
| DO Droplet (Hub) | `ssh root@REDACTED-IP`                                  | K3s control plane, vm-orchestration-service, vmproxy |
...
```bash
ssh root@REDACTED-IP "kubectl get nodes -o wide"
```

**Check node labels (including GPU mode):**

```bash
ssh root@REDACTED-IP "kubectl get nodes -o jsonpath='{range .items[*]}{.metadata.name}{\"\\t\"}{.metadata.labels}{\"\\n\"}{end}'"
```
...
```bash
ssh root@REDACTED-IP "kubectl get vm -n uvacompute"
```

**List running VM instances:**

```bash
ssh root@REDACTED-IP "kubectl get vmi -n uvacompute"
```

**Describe a specific VM (replace VM_NAME):**

```bash
ssh root@REDACTED-IP "kubectl describe vmi VM_NAME -n uvacompute"
```
...
```bash
ssh root@REDACTED-IP "kubectl get node aiworkstation -o jsonpath='{.metadata.labels.uvacompute\\.com/gpu-mode}'"
```
...
```bash
ssh root@REDACTED-IP "systemctl status vm-orchestration.service"
```

**View recent logs:**

```bash
ssh root@REDACTED-IP "journalctl -u vm-orchestration.service -n 100 --no-pager"
```

**Follow logs in real-time:**

```bash
ssh root@REDACTED-IP "journalctl -u vm-orchestration.service -f"
```

**Check startup/sync logs (useful after deploys):**

```bash
ssh root@REDACTED-IP "journalctl -u vm-orchestration.service --since '10 minutes ago' --no-pager | head -60"
```
...
```bash
ssh root@REDACTED-IP "kubectl get pods -n kubevirt"
```

**Check virt-handler on a node:**

```bash
ssh root@REDACTED-IP "kubectl logs -n kubevirt -l kubevirt.io=virt-handler --tail=50"
```

**Check virt-launcher pod for a specific VM:**

```bash
ssh root@REDACTED-IP "kubectl logs -n uvacompute -l kubevirt.io/domain=VM_NAME --tail=100"
```
...
```bash
ssh root@REDACTED-IP "systemctl status vmproxy"
```
...
```bash
ssh root@REDACTED-IP "kubectl get jobs,pods -n uvacompute -o wide"
```

**Check job details (creation time, spec):**

```bash
ssh root@REDACTED-IP "kubectl get job JOB_ID -n uvacompute -o yaml | head -50"
```

**Describe a pod for scheduling issues:**

```bash
ssh root@REDACTED-IP "kubectl describe pod POD_NAME -n uvacompute | head -40"
```

**Delete an orphan job:**

```bash
ssh root@REDACTED-IP "kubectl delete job JOB_ID -n uvacompute"
```
...
```bash
ssh root@REDACTED-IP "kubectl get events -n uvacompute --field-selector involvedObject.name=VM_NAME"
```

**Check cloud-init secret:**

```bash
ssh root@REDACTED-IP "kubectl get secret cloudinit-VM_ID -n uvacompute -o yaml"
```
...
```bash
ssh root@REDACTED-IP "kubectl get nodes -o jsonpath='{range .items[*]}{.metadata.name}{\"\\t\"}{.metadata.annotations.uvacompute\\.io/tunnel-port}{\"\\n\"}{end}'"
```

**Test SSH to node via tunnel (from hub):**

```bash
ssh root@REDACTED-IP "ssh -p TUNNEL_PORT localhost hostname"
```
...
3. Check node appears: `ssh root@REDACTED-IP "kubectl get nodes"`
