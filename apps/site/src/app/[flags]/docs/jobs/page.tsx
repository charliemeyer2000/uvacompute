      ...
      <section id="examples">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 mb-3">
              spin up an OpenAI-compatible API for any open-source model using{" "}
              <code className="bg-gray-100 px-1">--expose</code> to get an HTTPS
              endpoint:
            </p>
            <CodeBlock>
  ...
  -- vllm serve Qwen/Qwen2.5-7B-Instruct \\
    ...
    --max-model-len 4096`}</pre>
            ...
            <p className="text-sm text-gray-500 mt-2 mb-3">
              outputs an endpoint URL like{" "}
              <code className="bg-gray-100 px-1">
                https://abc123.uvacompute.com
              </code>
            </p>
            <p className="text-sm text-gray-600 mb-3">
              once logs show &quot;Application startup complete&quot;, query it:
            </p>
            <CodeBlock>
...
uva jobs run -g -c 4 -r 32 --expose 8000 -n llama-server \\
  ...
  -- vllm serve meta-llama/Llama-3.1-8B-Instruct \\
    --host 0.0.0.0 --port 8000`}</pre>
            ...
            <div className="bg-gray-50 border border-gray-200 p-3 mt-3 space-y-1">
              <p className="text-xs text-gray-500">
                <strong>tip:</strong> first run downloads ~15GB from HuggingFace
                (10-15 min). a 7B model uses ~14GB VRAM — the 5090&apos;s 32GB
                handles concurrent requests easily via continuous batching.
              </p>
            </div>
          </div>
        </div>
