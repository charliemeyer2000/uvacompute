...
│   │                                                                         │  │
│   │   VM Orchestration Service (Go)     SSH Proxy (ssh2incus)              │  │
│   │   ─────────────────────────────     ─────────────────────              │  │
│   │   POST /vms, /jobs                  Routes SSH to VMs                  │  │
│   │   Callbacks to site                 Port 2222                          │  │
│   │                                                                         │  │
...
- User connects to central jump host
- Jump host routes to correct node via reverse tunnel
- Similar to current `ssh2incus` but multi-node aware
...
### Phase 1: Single Node + Jobs

| Task                     | Description                            |
| ------------------------ | -------------------------------------- |
| Jobs in Convex           | Add `jobs` table, queries, mutations   |
| Jobs API (site)          | `/api/jobs/*` endpoints                |
| Jobs API (orchestration) | Create/cancel jobs via k8s             |
| Log streaming            | WebSocket/SSE from k8s → site → client |
| CLI commands             | `uva run`, `uva jobs`, `uva logs`      |
| Website UI               | Jobs list, log viewer                  |
| KubeVirtAdapter          | Replace Incus with KubeVirt            |

### Phase 2: Multi-Node

| Task                  | Description                  |
| --------------------- | ---------------------------- |
| k3s on DigitalOcean   | Control plane with public IP |
| SSH jump host         | Multi-node aware routing     |
| `uva node install`    | Join script for contributors |
| Node registration     | Track nodes in system        |
| Cross-node scheduling | k8s handles automatically    |

### Phase 3: Polish

| Task                  | Description                  |
| --------------------- | ---------------------------- |
| Log archival          | Move completed logs to R2/S3 |
| Kubernetes Dashboard  | Web admin UI                 |
| Tailscale integration | Optional direct connections  |
| Metrics/monitoring    | Resource usage, job stats    |
